...
# with internal scale augmentation during training and keypoint order randomization (with and without MuJoCo)
ccc run $GPUS_FILE python -m train ../config/vicos_towel+mujoco/train.py --config train_dataset.batch_size=24 train_dataset.workers=16 model.lr=1e-3 n_epochs=35 save_interval=5 train_dataset.keypoint_preprocesser_kwargs.apply_internal_scale_aug=True extra_str="_internal_scale_aug"  &
ccc run $GPUS_FILE python -m train ../config/vicos_towel/train.py --config train_dataset.batch_size=24 train_dataset.workers=16 model.lr=1e-3 n_epochs=100 save_interval=5 train_dataset.keypoint_preprocesser_kwargs.apply_internal_scale_aug=True extra_str="_internal_scale_aug"  &
